...
if (isset($_POST['uploud']) && !isset($_SESSION['data_uploaded'])) {
    ...
    $query = "INSERT INTO tugas (matkul, catatan, deadline) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sss", $matkul, $catatan, $deadline);
    
    if ($stmt->execute()) {
        $_SESSION['data_uploaded'] = true; // Tandai bahwa data sudah diupload
    } else {
        echo "Error: " . $stmt->error;
    }

    // Close the statement after insert operation
    $stmt->close();

    // Query untuk menghapus data yang sudah lewat deadline
    $now = date('Y-m-d H:i:s');
    $deleteQuery = "DELETE FROM tugas WHERE deadline < ?";
    $stmt = $conn->prepare($deleteQuery);
    $stmt->bind_param("s", $now);
    $stmt->execute();

    // Close the statement after delete operation
    $stmt->close();

    // Query untuk mengambil data terbaru
    $query_kalkulus = "SELECT * FROM tugas WHERE matkul = 'kalkulus'";
    $result_kalkulus = mysqli_query($conn, $query_kalkulus);

    $query_mtkd = "SELECT * FROM tugas WHERE matkul = 'mtk-diskrit'";
    $result_mtk_diskrit = mysqli_query($conn, $query_mtkd);

    $query_logmat = "SELECT * FROM tugas WHERE matkul = 'logmat'";
    $result_logmat = mysqli_query($conn, $query_logmat);

    $query_teori = "SELECT * FROM tugas WHERE matkul = 'alpro-teori'";
    $result_alprot = mysqli_query($conn, $query_teori);

    $query_prak = "SELECT * FROM tugas WHERE matkul = 'alpro-praktikum'";
    $result_alprop = mysqli_query($conn, $query_prak);
    
    $query_sta = "SELECT * FROM tugas WHERE matkul = 'statistika'";
    $result_statistika = mysqli_query($conn, $query_sta);

    $query_karak = "SELECT * FROM tugas WHERE matkul = 'Pkarakter'";
    $result_Pkarakter = mysqli_query($conn, $query_karak);

    $query_panca = "SELECT * FROM tugas WHERE matkul = 'pancasila'";
    $result_pancasila = mysqli_query($conn, $query_panca);

    $query_agama = "SELECT * FROM tugas WHERE matkul = 'agama-islam'";
    $result_agamai = mysqli_query($conn, $query_agama);
            ...
            <div class="container" style="margin-top: 1pc; font-family: 'Arial', sans-serif">
                <div class="row">
                    <div class="card-group">
                        <div class="card mb-5 shadow-lg" style="border-radius: 25px; padding: -10pc">
                                    ...
                                    <?php
                                           if ($result_kalkulus) {
                                            ...
                                            while ($row = mysqli_fetch_assoc($result_kalkulus)) {
                                                // Tampilkan data untuk matkul 'kalkulus'
                                                echo "<span style='font-size: 12px; color: #2d3436'> Batas: " . $row['deadline'] . "</span>";
                                                echo "<p class='mb-3'>" . $row['catatan'] . "</p>";
                                                    }
                                                } else {
                        ...
                        <div class="card mb-5 shadow-lg" style="border-radius: 25px">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text" style="text-align: start">
                                    <?php
                                              if($result_mtk_diskrit){
                                                while ($row = mysqli_fetch_assoc($result_mtk_diskrit)) {
                                                    if ($row['matkul'] == 'mtk-diskrit') {
                                                        echo "<span style='font-size: 12px; color: #2d3436'> Batas: " . $row['deadline'] . "</span>";
                                                        echo "<p class='mb-3'>" . $row['catatan'] . "</p>";
                                                    }
                                                }
                        ...
                        <div class="card mb-5 shadow-lg" style="border-radius: 25px">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text" style="text-align: start">
                                    <?php
                                              if($result_Pkarakter){
                                                while ($row = mysqli_fetch_assoc($result_Pkarakter)) {
                                                    if ($row['matkul'] == 'Pkarakter') {
                                                        echo "<span style='font-size: 12px; color: #2d3436'> Batas: " . $row['deadline'] . "</span>";
                                                        echo "<p class='mb-3'>" . $row['catatan'] . "</p>";
                                                    }
                                                }
                        ...
                        <div class="card mb-5 shadow-lg" style="border-radius: 25px">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text" style="text-align: start">
                                    <?php
                                              if($result_agamai){
                                                while ($row = mysqli_fetch_assoc($result_agamai)) {
                                                    if ($row['matkul'] == 'agama-islam') {
                                                        echo "<span style='font-size: 12px; color: #2d3436'> Batas: " . $row['deadline'] . "</span>";
                                                        echo "<p class='mb-3'>" . $row['catatan'] . "</p>";
                                                    }
                                                }
                        ...
                        <div class="card mb-5 shadow-lg" style="border-radius: 25px">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-text" style="text-align: start">
                                    <?php
                                              if($result_statistika){
                                                while ($row = mysqli_fetch_assoc($result_statistika)) {
                                                    if ($row['matkul'] == 'statistika') {
                                                        echo "<span style='font-size: 12px; color: #2d3436'> Batas: " . $row['deadline'] . "</span>";
                                                        echo "<p class='mb-3'>" . $row['catatan'] . "</p>";
                                                    }
                                                }
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="dashboar-admin.php" method="POST">
                        ...
                        <div class="form-floating">
                            <input type="datetime-local" class="form-control" id="floatingPassword" name="deadline"
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn" name="uploud"
